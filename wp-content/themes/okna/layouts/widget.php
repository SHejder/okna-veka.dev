...
foreach (array('suffix', 'panel', 'class', 'title_size', 'badge', 'icon', 'display') as $var) {
    $$var = isset($params[$var]) ? $params[$var] : null;
}

// Set default panel
if ($panel == '' && in_array($widget->position, array('top-a', 'top-b', 'top-c', 'bottom-a', 'bottom-b', 'bottom-c', 'bottom-d', 'main-top', 'main-bottom', 'sidebar-a', 'sidebar-b'))) {
    $panel = $this['config']->get("panel_default.{$widget->position}.panel", '');
}
// Set panel for specific positions
else if (in_array($widget->position, array('toolbar-r' ,'toolbar-l', 'footer-top', 'footer', 'offcanvas'))) {
    $panel = 'uk-panel';
}
...
if ($widget->menu) {
    ...
    if (isset($params['menu'])) {
        ...
        $widget->nav_settings["accordion"] = "{multiple:true, lists:'>li.uk-parent > a[href=\'#\'] + ul'}";
    } else {
        $renderer = 'nav';
...
if (in_array($widget->position, array('breadcrumbs', 'logo', 'logo-small', 'search', 'debug')) || (($widget->position == 'offcanvas') && $widget->menu)) {
    ...
    if ($widget->menu) {
        ...
        <ul class="uk-navbar-nav uk-hidden-small">
            <li class="uk-parent" data-uk-dropdown>
                <a href="#">'.$title.'</a>
                <div class="uk-dropdown uk-dropdown-navbar"><div class="uk-grid uk-dropdown-grid"><div class="uk-width-1-1 uk-text-center">'.$content.'</div></div></div>
            </li>
        </ul>';
    }
} else {
    ...
    // Set block class
    if (strpos($panel, 'uk-panel-box') === false) {
        if (preg_match('/tm-block-(default|light|dark)/i', $background, $result)) {
            $classes[] = "tm-block-{$result[1]}-panel";
        }
    }
